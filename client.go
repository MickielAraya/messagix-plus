...
func NewClient(platform types.Platform, cookies cookies.Cookies, proxy string, logger zerolog.Logger) (*Client, error) {
	cli := &Client{
		Logger:          logger,
		cookies:         cookies,
		lsRequests:      0,
...
func (c *Client) createHttpClient(proxy string) (tls_client.HttpClient, error) {
	options := []tls_client.HttpClientOption{
		...
		// tls_client.WithClientProfile(profiles.ConfirmedIos),
